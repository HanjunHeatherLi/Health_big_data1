...
def event_count_metrics(events, mortality):
    ...
    dead=events[events["patient_id"].isin(dead_list)]
    alive=events[~events["patient_id"].isin(dead_list)]

    dead_event_count=dead.groupby("patient_id").event_id.count()
    alive_event_count = alive.groupby("patient_id").event_id.count()
...
def encounter_count_metrics(events, mortality):
    ...
    dead_list = mortality["patient_id"].unique()
    dead = events[events["patient_id"].isin(dead_list)]
    dead_encounter_count=dead.groupby("patient_id")['timestamp'].nunique()

    alive = events[~events["patient_id"].isin(dead_list)]
    alive_encounter_count = alive.groupby("patient_id")['timestamp'].nunique()
...
def record_length_metrics(events, mortality):
    ...
    dead_list = mortality["patient_id"].unique()
    dead = events[events["patient_id"].isin(dead_list)]
    dead_rec=dead.groupby("patient_id").timestamp.max()-dead.groupby("patient_id").timestamp.min()
    dead_rec_len=dead_rec.dt.days
    #print(dead_rec_len)

    alive = events[~events["patient_id"].isin(dead_list)]
    alive_rec = alive.groupby("patient_id").timestamp.max() - alive.groupby("patient_id").timestamp.min()
    alive_rec_len =alive_rec.dt.days
    #print(alive_rec_len)
